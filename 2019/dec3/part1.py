...
def line_points(start, end):
    ...
    if sx == ex:
        ...
        if sy < ey:
            ...
            points = list(itertools.product([sx], range(ey, sy+1)))
    if sy == ey:
        # x changed
        if sx < ex:
            points = list(itertools.product(range(sx, ex+1), [sy]))
        else:
            points = list(itertools.product(range(ex, sx+1), [sy]))
...
def solve(lines_paths):
    ...
    return sorted([abs(i[0]) + abs(i[1]) for i in intersects])[1], intersects


for test in tests:
    result, data = solve(test["source"])
    if result == test["result"]:
        print("OK")
    else:
        print("NOK", test["result"], result, data)

result, data = solve(source)
print(result)
